...
def array_ordered
    array_sort = []
    array_sort = array.sort_by(&:length)
end

def array_a
    array_a = []
    array_a = array.each{|no_a| no_a.delete_prefix!("@")}
...
def array_five 
    array_five = []
    array_five = array_a.count{|five| five.length == 5}
...
def array_caps
    array_caps = []
    array_caps = array_a.count{|caps| caps.start_with?(/[A-Z]/)}
end

def array_lowcaps
    array_lowcaps = []
    array_lowcaps = array.each{|lowcaps| lowcaps.downcase!}
